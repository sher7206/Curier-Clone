...
@objc public class OTPFieldView: UIView {
    ...
    fileprivate func getOTPField(forIndex index: Int) -> OTPTextField {
        ...
        var fieldFrame = CGRect(x: 0, y: 0, width: fieldSize, height: 60)
        let x = ((UIScreen.main.bounds.width - 32) - (CGFloat(fieldsCount) * fieldSize + CGFloat(fieldsCount-1) * separatorSpace))/2
...
//            fieldFrame.origin.x =
            ...
            fieldFrame.origin.x = x + (fieldSize + separatorSpace) * CGFloat(index)
...
//            fieldFrame.origin.x = bounds.size.width / 2 - (CGFloat(fieldsCount / 2 - index) * fieldSize + CGFloat(fieldsCount / 2 - index - 1) * separatorSpace + separatorSpace / 2)
            fieldFrame.origin.x = (fieldSize + separatorSpace) * CGFloat(index)
        }
        
        fieldFrame.origin.y = 0//(bounds.size.height - fieldSize) / 2
    ...
    fileprivate func isPreviousFieldsEntered(forTextField textField: UITextField) -> Bool {
        ...
        if !shouldAllowIntermediateEditing {
            ...
            if let nextOTPField = nextOTPField {
                nextOTPField.textContentType = .oneTimeCode
